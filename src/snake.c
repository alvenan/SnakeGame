...
*/
blockStatus blockField[40 / BLOCK_SIDE_TOTAL_PIXELS]
                      [120 / BLOCK_SIDE_TOTAL_PIXELS] = {
                          EMPTY_BLOCK};  // limits 52/4 and 126/4 @TODO change
                                         // this to use MACROS instead numbers
...
static uint8_t convertLineToPage(uint8_t line) { return (line / 8); }

static uint8_t convertLineToBitPosition(uint8_t line) { return (line % 8); }
...
blockStatus checkBlockStatus(int blockXPosition, int blockYPosition) {
    if ((blockXPosition < (40 / BLOCK_SIDE_TOTAL_PIXELS)) &&
        (blockYPosition < (120 / BLOCK_SIDE_TOTAL_PIXELS))) {
...
void drawBlock(uint8_t blockXPosition, uint8_t blockYPosition,
               blockStatus blockTypeToDraw) {
    ...
    if (blockTypeToDraw == FULL_BLOCK) {
        if (checkBlockStatus(blockXPosition, blockYPosition) == EMPTY_BLOCK) {
            //@TODO move both if checks and all code to another function, and
            //just call this function here
...
int drawSnake(snake *s,
              snakeDirection dir)  //@todo remove dir, since snake type already
                                   //have direction member
{
    int delay_ms = 10;
    ...
    if (s->direction == dir) {
        switch (dir) {
            case (UP):
                s->head.snakeSegment.xCoordinate =
                    s->head.snakeSegment.xCoordinate - 1;
                break;

            case (RIGHT):
                s->head.snakeSegment.yCoordinate =
                    s->head.snakeSegment.yCoordinate + 1;
                break;
        ...
        switch (dir)  //@TODO CHANGE THIS TO IF AND ELSE: IF(UP || DOWN) IF(LEFT
                      //|| RIGHT)
        {
            case (UP):
    ...
    if (blockField[s->head.snakeSegment.xCoordinate]
                  [s->head.snakeSegment.yCoordinate] == FOOD_BLOCK) {
        score++;
        drawScore(score);
        screenHasFood = 0;

        blockField[s->head.snakeSegment.xCoordinate]
                  [s->head.snakeSegment.yCoordinate] = FULL_BLOCK;
    ...
    drawBlock(s->head.snakeSegment.xCoordinate,
              s->head.snakeSegment.yCoordinate, FULL_BLOCK);

    drawBlock(s->tail.snakeSegment.xCoordinate,
              s->tail.snakeSegment.yCoordinate, EMPTY_BLOCK);
